	...
	const NavTabs = () => {
		...
		return (
			<NavBar boxProps={{flexGrow: 0, borderColor: "main.700", borderTopWidth: 1}}>
				<NavTab isCurrent={location.pathname === "/ms"} icon={<Icon as={Ionicons} name="settings-sharp" size="2xs" />} link="/ms" key="NavTabSettings" />
				{range.map((n, i) => <NavTab isCurrent={location.pathname === "/ms/ms" + n} label={String(i+1)} link={"ms" + n} key={"Tab"+n} />)}
			</NavBar>
		);
	};
	return (
